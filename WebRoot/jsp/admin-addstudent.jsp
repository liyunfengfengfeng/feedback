...
<body>
  ...
  <div class="ui form main text container">
    <h1 class="ui header">添加学生</h1>
    <div class="ui message">${success}${error}</div>
    <form class="ui large form" action="newstudent.do" method="post">
      <div class="ui warning form segment">
        <div class="field">
          <label>学号</label>
          <input placeholder="学生登录账号" name="account" type="text">
        </div>
        <div class="field">
          <label>学生姓名</label>
          <input placeholder="学生姓名" name="stu_name" type="text">
        </div>
        
        <div class="field">
          <label>登录密码(默认为123456)</label>
          <input placeholder="默认为123456" name="password" type="password" value="123456">
        </div>
        <div>
        ...
        <div class="field">
          <label>所属班级</label>
          <select class="ui two dropdown" name="grade">
			<c:forEach items="${list}" var="data">
        	  	<option value="${data.id}">${data.gra_name}</option>
			</c:forEach>
          </select>
        </div>
        ...
        <div class="field">
          <label>出生日期</label>
          <input placeholder="格式yyyy-mm-dd" name="birthday" type="text">
        </div>
        <div class="field">
          <label>民族</label>
          <input placeholder="民族" name="nation" type="text" value="汉族">
        </div>
