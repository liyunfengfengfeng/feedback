...
<body>
  ...
  <div class="ui main text container">
    <h1 class="ui header">更新学生</h1>
    <div class="ui message">${success}${error}</div>
    <form class="ui large form" action="upstudent.do" method="post">
      <div class="ui warning form segment">
      	<input type="hidden" name="id" value="${data.id}">
        <div class="field">
          <label>学号</label>
          <input placeholder="学生登录账号" name="account" type="text" value="${data.account}">
        </div>
        <div class="field">
          <label>学生姓名</label>
          <input placeholder="学生姓名" name="stu_name" type="text" value="${data.stu_name}"/>
        </div>
        <div class="field">
        ...
        <div class="field">
          ...
          <select class="ui two dropdown" name="grade" >
			<c:forEach items="${list}" var="d">
				<c:if test="${data.grade.id == d.id }">
					<option value="${d.id}" selected="selected">${d.gra_name}</option>
				</c:if>
				<c:if test="${data.grade.id != d.id }">
					<option value="${d.id}">${d.gra_name}</option>
				</c:if>
			</c:forEach>
          </select>
        </div>
        ...
        <div class="ui primary submit button">更新</div>
